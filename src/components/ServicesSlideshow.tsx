      ...
      <DialogContent className="max-w-4xl h-[80vh] p-0 overflow-hidden">
        <div className="relative h-full">
          ...
          <div className="absolute top-4 left-4 right-4 z-10 flex justify-between items-center">
            <div className="text-sm text-gray-600 bg-white/80 backdrop-blur-sm px-3 py-1 rounded-full">
          ...
          <div className={`h-full ${currentSlideData.image} flex items-center justify-center p-8 transition-all duration-500`}>
            <div className="max-w-3xl text-center space-y-6 animate-fade-in">
              {currentSlideData.icon && (
                <div className={`flex items-center justify-center w-20 h-20 ${currentSlideData.available ? 'bg-[#145587]/10' : 'bg-gray-200'} rounded-3xl mx-auto relative`}>
              ...
              <div className="grid sm:grid-cols-2 gap-4 mt-8">
                {currentSlideData.features.map((feature, idx) => (
                  <div key={idx} className="flex items-center text-left bg-white/60 backdrop-blur-sm p-4 rounded-lg">
                    <div className={`w-3 h-3 ${currentSlideData.available ? 'bg-[#145587]' : 'bg-gray-400'} rounded-full mr-3 flex-shrink-0`}></div>
              ...
              {currentSlide === 0 && (
                <Button 
                  onClick={nextSlide}
                  className="bg-[#145587] hover:bg-[#145587]/90 text-white px-8 py-3 text-lg mt-8"
                >
              ...
              {slides.map((_, index) => (
                <button
                  ...
                  className={`w-3 h-3 rounded-full transition-colors ${
                    index === currentSlide ? 'bg-[#145587]' : 'bg-white/60'
                  }`}
                />
              ))}
            </div>
